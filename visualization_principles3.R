...
#1. Making a tile plot showing the rate of smallpox cases per population.
...
#2. Making a time series plot showing the rate of smallpox cases per population by state.
...
#3. Making a time series plot showing rates for all diseases in California
...
group_by(year, disease) %>% summarize(rate = sum(count)/sum(population)*10000) %>% 
